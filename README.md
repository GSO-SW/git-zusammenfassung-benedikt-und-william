...
- `git push` Wird zur arbeit zwischen local und remote benutzt und macht Veränderungen entgültig aus einer Kombination von "git merge" und "git fetch"
- `git pull` zieht den letzten commit aus einem remote repository zur Bearbeitung im local
- `git merge` Fügt zwei commits zusammen
- `git fetch` Wird benutzt um commits aus repositorys zu holen
- `git commit` Veränderungen an einem commit aus einem Repository